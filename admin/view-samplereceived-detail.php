...
$report=md5($report).time().$extension;
 move_uploaded_file($_FILES["report"]["tmp_name"],"images/".$report);
...
$query->bindParam(':aptid',$aptid,PDO::PARAM_STR); 
    ...
    $query->bindParam(':status',$status,PDO::PARAM_STR); 
       $query->execute();
      $sql= "update tblappointment set Report=:report, Status=:status,Remark=:remark where ID=:eid";
...
<head>
	...
	<title>Health Med Labs|| View Detail</title>
...
if($row->Status=="Delivered to Lab")
{
...
<td>
  <a href="images/<?php echo $row->Report;?>" target="_blank" width="200" height="150">Download Report</a>
</td>
<?php } else { ?>                  <td><?php  echo "Report Not Uploaded Yet";?>
                  </td>
                  <?php } ?>       
  </tr>
