...
if (strlen($_SESSION['odlmsaid']==0)) {
    header('location:logout.php');
        ...
        if(isset($_POST['submit']))
        {
            ...
            $sql="insert into tbllabtest(TestTitle,TestDescription,Testinterpretation,Price) values (:title,:discription,:interpretation,:price)";
            ...
            if ($LastInsertedId>0){
                echo '<script>alert("Test Deatils has been added")</script>';
                echo "<script>Window.location.href='add-test.php'</script>";    
            }
            else {
                echo '<script>alert("Somenthing went Wrong!! Please try again")</script>';
            }
        }
?>
...
<html lan="en">
    <head>
        <title>Health Med Labs Test Deatils</title>
